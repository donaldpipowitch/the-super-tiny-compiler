...
type NumberLiteralType = 'NumberLiteral';
...
type CallExpressionType = 'CallExpression';
...
type NodeType = ProgramType | CallExpressionType | StringLiteralType | NumberLiteralType;
export type Node = ProgramNode | CallExpressionNode | StringLiteralNode | NumberLiteralNode;
...
function traverser(ast: ProgramNode, visitor: Visitor) {
...
type NewNumberLiteralType = 'NumberLiteral';
interface NewNumberLiteralNode {
  type: NewNumberLiteralType;
  value: string;
}

type NewStringLiteralType = 'StringLiteral';
interface NewStringLiteralNode {
  type: NewStringLiteralType;
  value: string;
}

type NewExpressionStatementType = 'ExpressionStatement';
interface NewExpressionStatementNode {
  type: NewExpressionStatementType;
  expression: NewCallExpressionNode;
}

type NewCallExpressionType = 'CallExpression';
interface NewCallExpressionNode {
  type: NewCallExpressionType;
  callee: NewIdentifierNode;
  arguments: NewNode[];
}

type NewIdentifierType = 'Identifier';
interface NewIdentifierNode {
  type: NewIdentifierType;
  name: string;
}

type NewProgramType = 'Program';
interface NewProgramNode {
  type: NewProgramType;
  body: NewNode[];
}

type NewNodeType = NewProgramType | NewCallExpressionType | NewStringLiteralType | NewNumberLiteralType | NewExpressionStatementType | NewIdentifierType;
export type NewNode = NewProgramNode | NewCallExpressionNode | NewStringLiteralNode | NewNumberLiteralNode | NewExpressionStatementNode | NewIdentifierNode;

// So we have our transformer function which will accept the lisp ast.
function transformer(ast: ProgramNode): NewProgramNode {

  // We'll create a `newAst` which like our previous AST will have a program
  // node.
  let newAst: NewProgramNode = {
    type: 'Program',
    body: [],
  };
  ...
  traverser(ast, {
    ...
    NumberLiteral: {
      ...
      enter(node: NumberLiteralNode, parent) {
        // We'll create a new node also named `NumberLiteral` that we will push to
        // the parent context.
        (parent as any)._context.push({
          type: 'NumberLiteral',
          value: node.value,
        } as NewNumberLiteralNode);
      },
    },

    // Next we have `StringLiteral`
    StringLiteral: {
      enter(node: StringLiteralNode, parent) {
        (parent as any)._context.push({
          type: 'StringLiteral',
          value: node.value,
        } as NewStringLiteralNode);
      },
    },
    ...
    CallExpression: {
      enter(node: CallExpressionNode, parent) {
        ...
        // `Identifier`.
        let expression: NewCallExpressionNode | NewExpressionStatementNode = {
          type: 'CallExpression',
          callee: {
            type: 'Identifier',
            name: node.name,
...
function codeGenerator(node: NewNode): string {
